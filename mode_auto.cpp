...
Vector2f ModeAuto::calculate_new_location(float lat1, float lon1, float distance, int bearing) {
    ...
    float bearing_rad = deg2rad(bearing);
    float lat2_rad = asinf(sinf(lat1_rad) * cosf(distance / R) + cosf(lat1_rad) * sinf(distance / R) * cosf(bearing_rad));
    float lon2_rad = lon1_rad + atan2f(sinf(bearing_rad) * sinf(distance / R) * cosf(lat1_rad), cosf(distance / R) - sinf(lat1_rad) * sinf(lat2_rad));
    float lat2 = rad2deg(lat2_rad);
    float lon2 = rad2deg(lon2_rad);
    return { lat2, lon2 };
}
...
Location ModeAuto::set_turn_wp(const Vector2f curr_loc, const Vector2f next_loc) {
    ...
    float thrust_y = pos_control->get_thrust_vector().y;
    ...
    float angle_rad;
    float distance_external_sphere;
    Vector2f external_point;
